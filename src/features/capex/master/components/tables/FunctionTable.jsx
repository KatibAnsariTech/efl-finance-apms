...
export default function FunctionTable({ handleEdit: parentHandleEdit, handleDelete: parentHandleDelete, refreshTrigger, tabChangeTrigger }) {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await userRequest.get("cpx/getBusinessFunctions", {
        params: {
          page: page + 1,
          limit: rowsPerPage,
        },
      ...
      showErrorMessage(error, "Error fetching Function data", swal);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <DataGrid
        key={`function-table-${tabChangeTrigger}`}
        rows={data}
        columns={columns}
        loading={loading}
        pagination
