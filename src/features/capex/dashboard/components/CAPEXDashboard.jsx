...
export default function CAPEXDashboard() {
  ...
  const getData = async () => {
    try {
      const response = await userRequest.get('/capex/getFormStatistics');
  ...
  const getChartData = async () => {
    try {
      setLoading(true);
      const response = await userRequest.get('/capex/getFormBarChartData', {
  ...
  const getPieChartData = async () => {
    try {
      setPieLoading(true);
      const response = await userRequest.get('/capex/getRegionPieChartData', {
        ...
        <Grid container spacing={3}>
          <Grid xs={12} sm={6} md={3}>
            <AppWidgetSummary
              ...
              total={cardData && cardData.todayCAPEXEntries}
              color="success"
          ...
          <Grid xs={12} sm={6} md={3}>
            <AppWidgetSummary
              ...
              total={cardData && cardData.totalCAPEXEntries}
              color="info"
