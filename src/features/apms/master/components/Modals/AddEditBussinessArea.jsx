...
function AddEditBussinessArea({ handleClose, open, editData: accountTypeData, getData }) {
  ...
  const handleSaveData = async (data) => {
    ...
    try {
      ...
      if (accountTypeData?._id) {
        await userRequest.put(`/apms/updateMaster/${accountTypeData._id}`, formattedData);
        getData();
        swal("Updated!", "Account Type data updated successfully!", "success");
      } else {
        await userRequest.post("/apms/createMasters", formattedData);
        getData();
      ...
      >
        ...
        >
          <span style={{ fontSize: "24px", fontWeight: "bolder" }}>
            {accountTypeData ? "Edit Business Area" : "Add Business Area"}
          </span>
          <RxCross2
            onClick={handleClose}
            style={{
...
export default AddEditBussinessArea;
