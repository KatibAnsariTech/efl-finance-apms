...
function AddEditSalesGroup({ handleClose, open, editData, getData }) {
  ...
  useEffect(() => {
    async function fetchChannels() {
      try {
        const res = await userRequest.get("/admin/getMasters?key=SalesOffice");
        setSalesOfficeOptions(res?.data?.data?.masters || []);
      } catch (err) {
  ...
  const handleSaveData = async (data) => {
    ...
    try {
      if (editData?._id) {
        ...
        getData();
        swal("Updated!", "Sales group data updated successfully!", "success");
      } else {
        await userRequest.post("/admin/createMasters", formattedData);
        // console.log("Saving new request type data:", formattedData);
        getData();
        swal("Success!", "Sales group data saved successfully!", "success");
      }
      reset();
      handleClose();
    } catch (error) {
      ...
      >
        ...
        >
          <span style={{ fontSize: "24px", fontWeight: "bolder" }}>
            {editData ? "Edit Sales Group" : "Add Sales Group"}
          </span>
          <RxCross2
            onClick={handleClose}
            style={{
        ...
        >
          ...
          {/* <FormControl fullWidth>
            <InputLabel id="channels-label">Sales Office</InputLabel>
            <Controller
              name="other"
              control={control}
              defaultValue={[]}
                ...
                <Select
                  ...
                  renderValue={(selected) => (
                    <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1 }}>
                      {selected.map((id) => {
                        const label =
                          salesOfficeOptions.find((opt) => opt._id === id)?.value ||
                          id;
                        return (
                          <Chip
                            key={id}
          ...
          <FormControl fullWidth>
            <InputLabel id="sales-office-label">Sales Office</InputLabel>
            <Controller
              name="other"
              control={control}
              defaultValue=""
...
export default AddEditSalesGroup;
