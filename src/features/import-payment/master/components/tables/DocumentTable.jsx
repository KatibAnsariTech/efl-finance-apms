...
export default function DocumentTable({ handleEdit, handleDelete, refreshTrigger }) {
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await userRequest.get(
        `/imt/getDocuments?page=${paginationModel.page + 1}&limit=${
          paginationModel.pageSize
        }`
      );
      if (response.data.success) {
        const mappedData = response.data.data.documents.map((item, index) => ({
          id: item._id,
          sno: paginationModel.page * paginationModel.pageSize + index + 1,
          importtype: item?.importType?.value || "-",
          document: item?.name || "-",
          ...item,
        }));
        setData(mappedData);
