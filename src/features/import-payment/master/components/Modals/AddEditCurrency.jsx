...
function AddEditCurrency({ handleClose, open, editData: accountTypeData, getData }) {
  const [loading, setLoading] = React.useState(false);
  const { register, handleSubmit, reset, setValue } = useForm();

  React.useEffect(() => {
    if (accountTypeData) {
      setValue("currencyname", accountTypeData.currencyname || "");
      setValue("exchangerate", accountTypeData.exchangerate || "");
    } else {
      reset();
    }
  ...
  const handleSaveData = async (data) => {
    ...
    try {
      ...
      if (accountTypeData?._id) {
        await userRequest.put(`/imt/updateCurrency/${accountTypeData._id}`, formattedData);
        getData();
        swal("Updated!", "Account Type data updated successfully!", "success");
      } else {
        await userRequest.post("/imt/createCurrency", formattedData);
        getData();
      ...
      >
        ...
        >
          <span style={{ fontSize: "24px", fontWeight: "bolder" }}>
            {accountTypeData ? "Edit Currency" : "Add Currency"}
          </span>
          <RxCross2
            onClick={handleClose}
            style={{
